...
## TODO: BUG — KEY_BUFFS list is incomplete; missing buffs and name variants

> BUG: the buff coverage section is missing buffs like Prayer of Fortitude / Greater Prayer of Fortitude and probably others. Audit a sample benchmark raid's buff aura data to pull the full set of important group and raid-wide buffs and their Greater/variant counterparts, then replace KEY_BUFFS with a complete, correctly-aliased list.

A raid leader reading the Buff Coverage section needs to trust that every buff that *should* be on the raid is tracked — a missing entry (Prayer of Fortitude, etc.) silently drops a real coverage lever. The benchmark raid's aura data is the right sample — it's a known-good raid and will show exactly which buffs were active and under what names they log.
